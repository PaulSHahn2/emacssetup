...
;; possible free desktop locations for desktop files
...
(defun exwmsetup/goto-previous-frame ()
  "Go to the frame one less than current by index, or wrap to 0 if last frame."
  (interactive)
  (defvar-local my-what-workspace 0)
  (if (= exwm-workspace-current-index 0)
       (setq my-what-workspace (- exwm-workspace-number 1))
    (setq my-what-workspace (- exwm-workspace-current-index 1))
   )
  (exwm-workspace-switch-create my-what-workspace)
  (notifications-notify :title "workspace" :body (number-to-string exwm-workspace-current-index) :timeout 1000)
)

(defun exwmsetup/goto-next-frame ()
  "Go to the next frame by index, or wrap to 0 if last frame."
  (interactive)
  (defvar-local my-what-workspace 0)
  (if (= exwm-workspace-current-index (- exwm-workspace-number 1))
      (setq my-what-workspace 0)
    (setq my-what-workspace (+ exwm-workspace-current-index 1))
  )
  (exwm-workspace-switch-create my-what-workspace)
  (notifications-notify :title "workspace" :body (number-to-string exwm-workspace-current-index) :timeout 1000)
...
(defun exwmsetup/get-xdg-desktop-file-data (desktop-file)
  "For a given DESKTOP-FILE, get the xdg data in the file."
  ;;(message "Found xdg desktop application file: %s" desktop-file)
  (defvar-local local-temp-hash (xdg-desktop-read-file desktop-file))
  (defvar-local local-class-name (elt (split-string (gethash "StartupWMClass" local-temp-hash (gethash "Exec" local-temp-hash "!KEY-MISSING")) " " 1) 0))
  (unless (string-equal local-class-name  "!KEY-MISSING")
    (puthash local-class-name local-temp-hash exwmsetup/exwm-xdg-program-categories-hash)))
...
(defun exwmsetup/get-xdg-desktop-category-list (exwm-class-name)
  "Get the list of xdg categories the given application specified by EXWM-CLASS-NAME belongs too."
  (split-string (gethash "Categories" (gethash exwm-class-name exwmsetup/exwm-xdg-program-categories-hash) "Unknown;") ";" 1))
...
(defun exwmsetup/configure-window-by-class ()
  "When an x windows program is started, compare it to class name
and configure custom behavior if a match is found."
  (interactive)
  (message "Found window: '%s'" exwm-class-name)
  (pcase exwm-class-name
    ("Firefox" (exwm-workspace-move-window 2))
    ("Chromium" (exwm-workspace-move-window 2))
    ("Thunderbird" (exwm-workspace-move-window 2))
    ("libreoffice-calc" (exwm-workspace-move-window 4))
    ("libreoffice-writer" (exwm-workspace-move-window 4))
    ("libreoffice-startcenter" (exwm-workspace-move-window 4))
    ("libreoffice-draw" (exwm-workspace-move-window 4))
    ("libreoffice-impress" (exwm-workspace-move-window 4))
  )
)
...
(use-package exwm
  ...
  (add-hook 'exwm-manage-finish-hook #'exwmsetup/configure-window-by-class)
  ...
  (setq exwm-input-global-keys
        ...
        `(
          ([?\s-r] . exwm-reset)
          ([s-left] . exwmsetup/goto-previous-frame)
          ([s-right] . exwmsetup/goto-next-frame)
  ...
  ;; Workspace 0 (left): org-mode windows. Mail inside emacs.
  ;; Workspace 1 (center): emacs programming: lisp, python, C, markdown, etc.
  ;; Workspace 2 (right): Browsers, Thunderbird mail reader, IM chat, and similar X programs.
  ;; Workspace 3 (not shown): Simple games, blender, krita, inkscape
  ;; Workspace 4 (not shown): Office Productivity Junk
  ;; Workspace 5 (not shown): Private
